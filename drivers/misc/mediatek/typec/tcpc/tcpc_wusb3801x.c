...
static void wusb3801_irq_work_handler(struct kthread_work *work)
{
	...
	if (sts & WUSB3801_INT_DETACH) {
		...
		/*When the OTG plugout, wusb3801 will read the vbus device status.
		If the vbus has not returned to below the threshold for about 40ms after plugging out, an extra insertion interrupt will be generated.
		Therefore, a 200ms delay is added when pluggingout to shield this additional interrupt. */
		msleep(200);
	}
